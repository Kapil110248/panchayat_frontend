...
export default function ClerkManagement() {
  ...
  const handleAddClerk = async (e) => {
    ...
    try {
      ...
      setFormData({ full_name: "", email: "", mobile: "", password: "", village: "Your Village", permissions: [] });
      fetchClerks();
    } catch (err) {
  ...
  const handleEditClick = (clerk) => {
    setFormData({
      ...
      email: clerk.email,
      mobile: clerk.mobile,
      village: clerk.village === 'Panchayat Office' ? 'Your Village' : clerk.village,
      password: '',
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input required type="text" className="w-full mt-1 p-4 bg-slate-50 border-2 border-transparent focus:border-primary/30 focus:bg-white rounded-2xl text-sm font-bold text-slate-700 transition-all outline-none" value={formData.village} onChange={e => setFormData({...formData, village: e.target.value})} placeholder="Your Village" />
                    </div>
                  </div>
                  <div>
              ...
              <form onSubmit={handleEditClerk} className="space-y-5 bg-white p-6 rounded-[2rem] shadow-xl border border-slate-100 relative">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Assigned Village</label>
                      <input required type="text" className="w-full mt-1 p-4 bg-slate-50 border-2 border-transparent focus:border-amber-500/30 focus:bg-white rounded-2xl text-sm font-bold text-slate-700 transition-all outline-none" value={formData.village} onChange={e => setFormData({...formData, village: e.target.value})} placeholder="Your Village" />
                    </div>
                  </div>
                  <div>
